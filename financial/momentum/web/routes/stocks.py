...
from financial.momentum.web.utils.loader import display_name_for
...
def models_by_ticker(lang, ticker):
    ...
    return render_template(f"{lang}/stocks_ticker.html", ticker=ticker, ticker_name=display_name_for(ticker), models=models, lang=lang)
...
def model_detail(lang, ticker, model_id):
    ...
    if details["predictions"]:
        ...
        plot_html = None
    return render_template(f"{lang}/model_detail.html", lang=lang, ticker=ticker, ticker_name=display_name_for(ticker), model_id=model_id, details=details, plot_html=plot_html)
